#!/usr/bin/env python3
"""
Working Research Paper Pipeline API
...
def download_pdfs():
    """Simplified PDF download endpoint"""
    try:
        data = request.json
        papers = data.get('papers', [])
        ...
        print(f"PDF download requested for {len(papers)} papers...")
        ...
        for i, paper in enumerate(papers):
            ...
            # Simplified PDF search - just check for arXiv papers for now
        ...
        summary = {
            ...
            'download_success_rate': f"{(found_pdfs/len(papers)*100):.1f}%" if papers else "0%",
            'results': results
        }
